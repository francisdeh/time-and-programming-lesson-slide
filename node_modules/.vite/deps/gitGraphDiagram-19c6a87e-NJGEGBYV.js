...
// node_modules/mermaid/dist/gitGraphDiagram-19c6a87e.js
var me = function() {
...
Expecting ` + se.join(", ") + ", got '" + (this.terminals_[P] || P) + "'" : he = "Parse error on line " + (re + 1) + ": Unexpected " + (P == pe ? "end of input" : "'" + (this.terminals_[P] || P) + "'"), this.parseError(he, {
    ...
    var q = {
      ...
      setInput: function(u, g) {
        return this.yy = g || this.yy || {}, this._input = u, this._more = this._backtrack = this.done = false, this.yylineno = this.yyleng = 0, this.yytext = this.matched = this.match = "", this.conditionStack = ["INITIAL"], this.yylloc = {
          first_line: 1,
          first_column: 0,
          last_line: 1,
          last_column: 0
...
` + g + "^";
      ...
      next: function() {
        ...
        for (var w = this._currentRules(), e = 0; e < w.length; e++)
          if (x = this._input.match(this.rules[w[e]]), x && (!g || x[0].length > g[0].length)) {
            if (g = x, s = e, this.options.backtrack_lexer) {
              if (u = this.test_match(x, w[e]), u !== false)
                return u;
              if (this._backtrack) {
                g = false;
                continue;
        ...
        return g ? (u = this.test_match(g, w[s]), u !== false ? u : false) : this._input === "" ? this.EOF : this.parseError("Lexical error on line " + (this.yylineno + 1) + `. Unrecognized text.
` + this.showPosition(), {
          text: "",
          token: null,
          line: this.yylineno
...
me.parser = me;
var Ve = me;
var ae = Xn().gitGraph.mainBranchName;
var De = Xn().gitGraph.mainBranchOrder;
var M = {};
var I = null;
var $ = {};
$[ae] = { name: ae, order: De };
...
function ze(r, n) {
  ...
  return r.reduce((m, a) => {
    const c = n(a);
    return o[c] || (o[c] = true, m.push(a)), m;
  }, []);
}
var je = function(r) {
  ke = r;
...
var qe = function(r) {
  ...
  try {
    ...
    A.error("error while parsing gitGraph options", n.message);
  }
};
var Ye = function() {
  return xe;
};
var Fe = function(r, n, o, m) {
  A.debug("Entering commit:", r, n, o, m), n = In.sanitizeText(n, Xn()), r = In.sanitizeText(r, Xn()), m = In.sanitizeText(m, Xn());
  const a = {
    id: n || U + "-" + ue(),
    message: r,
    seq: U++,
  ...
  I = a, M[a.id] = a, L[O] = a.id, A.debug("in pushCommit " + a.id);
};
var Ke = function(r, n) {
  if (r = In.sanitizeText(r, Xn()), L[r] === void 0)
    L[r] = I != null ? I.id : null, $[r] = { name: r, order: n ? parseInt(n, 10) : null }, ye(r), A.debug("in createBranch");
  else {
    let o = new Error(
...
var Ue = function(r, n, o, m) {
  r = In.sanitizeText(r, Xn()), n = In.sanitizeText(n, Xn());
  ...
  I = l, M[l.id] = l, L[O] = l.id, A.debug(L), A.debug("in mergeBranch");
};
var We = function(r, n, o, m) {
  if (A.debug("Entering cherryPick:", r, n, o), r = In.sanitizeText(r, Xn()), n = In.sanitizeText(n, Xn()), o = In.sanitizeText(o, Xn()), m = In.sanitizeText(m, Xn()), !r || M[r] === void 0) {
    let l = new Error(
  ...
  if (!n || M[n] === void 0) {
    ...
    I = h, M[h.id] = h, L[O] = h.id, A.debug(L), A.debug("in cherryPick");
  }
};
var ye = function(r) {
  if (r = In.sanitizeText(r, Xn()), L[r] === void 0) {
...
function _e(r) {
  ...
  for (let a in L)
    L[a] === n.id && m.push(a);
  if (A.debug(m.join(" ")), n.parents && n.parents.length == 2) {
...
var Qe = function() {
  M = {}, I = null;
  let r = Xn().gitGraph.mainBranchName, n = Xn().gitGraph.mainBranchOrder;
  L = {}, L[r] = null, $ = {}, $[r] = { name: r, order: n }, O = r, U = 0, v_();
};
var Xe = function() {
...
var de = (r, n, o) => {
  const m = Xn().gitGraph, a = r.append("g").attr("class", "commit-bullets"), c = r.append("g").attr("class", "commit-labels");
  let l = 0;
  A2 === "TB" && (l = 30), Object.keys(n).sort((k, i) => n[k].seq - n[i].seq).forEach((k) => {
    const i = n[k], b = A2 === "TB" ? l + 10 : R[i.branch].pos, f = A2 === "TB" ? R[i.branch].pos : l + 10;
    if (o) {
    ...
    if (A2 === "TB" ? ee[i.id] = { x: f, y: l + 10 } : ee[i.id] = { x: l + 10, y: b }, o) {
      if (i.type !== N.CHERRY_PICK && (i.customId && i.type === N.MERGE || i.type !== N.MERGE) && m.showCommitLabel) {
        ...
        if (E.attr("x", l + 10 - T.width / 2 - 2).attr("y", b + 13.5).attr("width", T.width + 2 * 2).attr("height", T.height + 2 * 2), A2 === "TB" && (E.attr("x", f - (T.width + 4 * 4 + 5)).attr("y", b - 12), H.attr("x", f - (T.width + 4 * 4)).attr("y", b + T.height - 12)), A2 !== "TB" && H.attr("x", l + 10 - T.width / 2), m.rotateCommitLabel)
          if (A2 === "TB")
      ...
      if (i.tag) {
        ...
        d.attr("class", "tag-label-bkg").attr(
          ...
          ${l + 10 + T.width / 2 + 4},${G + B + 2}
          ${l + 10 - T.width / 2 - 4},${G + B + 2}`
        ), E.attr("cx", l - T.width / 2 + 4 / 2).attr("cy", G).attr("r", 1.5).attr("class", "tag-hole"), A2 === "TB" && (d.attr("class", "tag-label-bkg").attr(
          "points",
          `
            ${f},${l + 2}
            ${f},${l - 2}
...
var at = (r, n, o, m, a) => {
  const l = (A2 === "TB" ? o.x < m.x : o.y < m.y) ? n.branch : r.branch, h = (k) => k.branch === l, y = (k) => k.seq > r.seq && k.seq < n.seq;
...
var ot = (r, n) => {
  const o = Xn().gitGraph, m = r.append("g");
  n.forEach((a, c) => {
    const l = c % K, h = R[a.name].pos, y = m.append("line");
    y.attr("x1", 0), y.attr("y1", h), y.attr("x2", te), y.attr("y2", h), y.attr("class", "branch branch" + l), A2 === "TB" && (y.attr("y1", 30), y.attr("x1", h), y.attr("y2", te), y.attr("x2", h)), ne.push(h);
    let k = a.name;
    ...
    let p = i.getBBox();
    b.attr("class", "branchLabelBkg label" + l).attr("rx", 4).attr("ry", 4).attr("x", -p.width - 4 - (o.rotateCommitLabel === true ? 30 : 0)).attr("y", -p.height / 2 + 8).attr("width", p.width + 18).attr("height", p.height + 4), _.attr(
      "transform",
      "translate(" + (-p.width - 14 - (o.rotateCommitLabel === true ? 30 : 0)) + ", " + (h - p.height / 2 - 1) + ")"
    ), A2 === "TB" && (b.attr("x", h - p.width / 2 - 10).attr("y", 0), _.attr("transform", "translate(" + (h - p.width / 2 - 5) + ", 0)")), A2 !== "TB" && b.attr("transform", "translate(-19, " + (h - p.height / 2) + ")");
  });
};
var lt = function(r, n, o, m) {
  st();
  const a = Xn(), c = a.gitGraph;
  A.debug("in gitgraph renderer", r + `
`, "id:", n, o), X = m.db.getCommits();
  const l = m.db.getBranchesAsObjArray();
  A2 = m.db.getDirection();
  const h = Ct(`[id="${n}"]`);
  let y = 0;
  l.forEach((k, i) => {
    ...
    let d = b.getBBox();
    R[k.name] = { pos: y, index: i }, y += 50 + (c.rotateCommitLabel ? 40 : 0) + (A2 === "TB" ? d.width / 2 : 0), p.remove(), _.remove(), f.remove();
  }), de(h, X, false), c.showBranches && ot(h, l), ct(h, X), de(h, X, true), Ze.insertTitle(
    h,
    "gitTitleText",
    c.titleTopMargin,
...
//# sourceMappingURL=gitGraphDiagram-19c6a87e-NJGEGBYV.js.map
